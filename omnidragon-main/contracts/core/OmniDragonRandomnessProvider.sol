...
contract OmniDragonRandomnessProvider is Ownable, ReentrancyGuard, IOmniDragonVRFConsumer {
    ...
    event PoolGeneratedWithMinimalEntropy(uint256 minimalSeed);
    ...
    function drawFromRandomnessPool() external nonReentrant returns (uint256 randomness) {
        ...
        // Mix with current block data for additional uniqueness
    ...
    function _generateRandomnessPool(uint256 chainlinkSeed, bool drandOnly) internal {
        ...
        // If we have insufficient entropy, use a minimal seed approach
        if (entropySources.length < 2 && chainlinkSeed == 0) {
            // Use block data as minimal entropy (less secure but prevents revert)
            // This should only happen during initial deployment before Drand networks are configured
            ...
            // Add minimal seed to sources
            assembly {
                mstore(entropySources, 1)
            }
            entropySources[0] = minimalSeed;

            // Log warning event
            emit PoolGeneratedWithMinimalEntropy(minimalSeed);
        }
