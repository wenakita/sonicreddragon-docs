 ...
 * with proper error handling, dark/light theme support, and SPA navigation handling.
 * It consolidates functionality from mermaidInit.js and optimizedMermaidInit.js.
 */
 ...
 */
function initializeMermaid(userConfig = {}) {
  ...
  const attemptInterval = config.attemptInterval;
  ...
  function attemptInitialization() {
    ...
    try {
      ...
      if (typeof window.mermaid !== 'undefined') {
        ...
        // Configure mermaid with merged settings
...
/**
 * Add CSS styles for Mermaid diagrams
 */
...
export default function(userConfig = {}) {
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => initializeMermaid(userConfig));
  } else {
    initializeMermaid(userConfig);
  }
  
  // Re-initialize on route change for SPA navigation
  document.addEventListener('docusaurus.routeDidUpdate', () => {
    // Wait for DOM to update
    setTimeout(() => initializeMermaid(userConfig), 200);
  });
}
