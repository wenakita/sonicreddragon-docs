...
contract ve69LP is Ownable, ReentrancyGuard, Ive69LP {
    ...
    /**
     * @dev Calculate voting power based on amount and lock time - CRITICAL FIX: Proper scaling
    ...
    function calculateVotingPower(uint256 _amount, uint256 _unlockTime) public view returns (uint256) {
        ...
        // CRITICAL FIX: Use consistent 1e18 scaling throughout
        // Calculate normalized boost multiplier (returns value scaled by 1e18)
        uint256 normalizedBoost = ve69LPMath.calculateNormalizedBoostMultiplier(
            lockDuration,
            MAX_LOCK_TIME
        );

        // CRITICAL FIX: Convert maxVP from BPS to 1e18 scale for consistent math
        uint256 maxBoostMultiplier = (maxVP * PRECISION) / 10000; // Convert BPS to 1e18 scale

        // Apply boost cap (cannot exceed maxBoostMultiplier)
        if (normalizedBoost > maxBoostMultiplier) {
            normalizedBoost = maxBoostMultiplier;
        }

        // CRITICAL FIX: Consistent scaling - base amount + boost
        // Base voting power (1x) + boost (scaled properly)
        uint256 baseVotingPower = _amount; // 1x voting power
        uint256 boostVotingPower = (_amount * normalizedBoost) / PRECISION;
